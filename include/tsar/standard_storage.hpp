...
namespace tsar {
  ...
  public:
    ...
    constexpr  static std::size_t size_in_bytes() {
      return layout[size()-1].offset + layout[size()-1].size;
    }
  ...
  private:
    ...
    constexpr static std::size_t nth_offset(std::size_t idx) {
      return layout[indexing[idx]].offset;
    }
